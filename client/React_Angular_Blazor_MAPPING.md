...
| **라우팅 링크**                          | `<Link to="/path">`<br/>`useNavigate()`              | `[routerLink]="'/path'"`<br/>`routerLink`                                                         | `<NavLink href="/path">`                                                                                                |
| **라우터 서비스(프로그래밍 네비게이션)** | `useNavigate()`<br/>`navigate('/path')`              | `Router` 서비스<br/>`this.router.navigate(['/path'])`                                             | `NavigationManager.NavigateTo("/path")`                                                                                 |
...
| **입력값/상태 변화에 반응하는 로직**     | `useEffect(() => ..., [value])`                      | `@Input() set ...`, `ngOnChanges`,<br/>`Signal effect()`,<br/>`Observable.subscribe()/async pipe` | `OnParametersSet`, 파라미터 property setter (단, 임의 변수 변화는 직접 구현 필요)                                       |
